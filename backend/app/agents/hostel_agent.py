...
def get_student_hostel_info(db: Session, student_id: str) -> Dict:
    ...
    logger.info(f"Hostel Agent: get_student_hostel_info student_id={student_id}")
    try:
        ...
        if not allocation:
            logger.info(f"Hostel Agent: No hostel allocation found for student_id={student_id}")
            return {
        ...
        if not room or not hostel:
            logger.warning(f"Hostel Agent: Allocation found but room/hostel relation is null for student_id={student_id}")
            return {
                "student_id": student_id,
                "name": student.name,
                "hostel_allocated": False,
                "message": "No active room or hostel details allocated."
            }
        ...
        logger.exception(f"Hostel Agent error in get_student_hostel_info: {e}")
        return {
            "student_id": student_id,
            "hostel_allocated": False,
            "message": f"Error fetching hostel info: {str(e)}"
        }


def file_complaint(db: Session, student_id: str, complaint_text: str) -> Dict:
    """File a new hostel complaint with auto-classification."""
    logger.info(f"Hostel Agent: file_complaint student_id={student_id}")
    try:
        student = db.query(Student).filter(Student.student_id == student_id).first()
        if not student:
            return {"error": "Student not found"}
        ...
        logger.exception(f"Hostel Agent error in file_complaint: {e}")
        return {"error": f"Failed to file complaint: {str(e)}"}


def get_complaint_history(db: Session, student_id: str) -> List[Dict]:
    """Get all complaints for a student."""
    logger.info(f"Hostel Agent: get_complaint_history student_id={student_id}")
    try:
        student = db.query(Student).filter(Student.student_id == student_id).first()
        if not student:
            return []
        ...
        logger.exception(f"Hostel Agent error in get_complaint_history: {e}")
        return []


def get_mess_menu(db: Session, day: Optional[str] = None) -> List[Dict]:
    """Get mess menu for a specific day or all days."""
    logger.info(f"Hostel Agent: get_mess_menu day={day}")
    try:
        ...
        logger.exception(f"Hostel Agent error in get_mess_menu: {e}")
        return []


def get_hostel_occupancy(db: Session) -> Dict:
    """Room occupancy statistics."""
    logger.info("Hostel Agent: get_hostel_occupancy")
    try:
        ...
        logger.exception(f"Hostel Agent error in get_hostel_occupancy: {e}")
        return {
            "summary": {"total_rooms": 0, "occupied": 0, "available": 0, "overall_occupancy_pct": 0.0},
            "hostels": []
        }
