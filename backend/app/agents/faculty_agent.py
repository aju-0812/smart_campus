...
def handle_faculty_query(db: Session, entities: dict) -> dict:
    ...
    # Check if the text matches a department or course first
    ...
    if search_term and not fac_name:
        ...
        if dept:
            ...
            return {
                ...
                "hod": getattr(db.query(Faculty).filter(Faculty.id == dept.hod_id).first(), "faculty_name", "N/A"),
    ...
    if query_type == "all_faculty":
        ...
        return {"faculty_list": [f.faculty_name for f in facs[:20]]} # Limiting for context size
