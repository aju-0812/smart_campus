...
def handle_course_catalog_query(db: Session, entities: dict) -> dict:
    ...
    """
    logger.info(f"Course Catalog Agent: Incoming entities={entities}")
    try:
        ...
        logger.exception(f"Course Catalog Agent error: {e}")
        return {"error": f"An error occurred while fetching course catalog: {str(e)}"}
