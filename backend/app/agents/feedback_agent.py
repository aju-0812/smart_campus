...
def submit_feedback(
    ...
    logger.info(f"Feedback Agent: submit_feedback from student_id={student_id}")
    try:
        ...
        logger.exception(f"Feedback Agent error in submit_feedback: {e}")
...
def get_faculty_analytics(db: Session, faculty_id: int) -> Dict:
    ...
    logger.info(f"Feedback Agent: get_faculty_analytics for faculty_id={faculty_id}")
    try:
        ...
        if not responses:
            return {
                ...
                "department": faculty.department_ref.department_name if faculty.department_ref else "N/A",
                "total_responses": 0,
                "avg_rating": 0.0,
                "sentiment_distribution": {"Positive": 0, "Negative": 0, "Neutral": 0},
                "sentiment_pct": {"Positive": 0.0, "Negative": 0.0, "Neutral": 0.0},
                "top_keywords": [],
                "rating_trend": "Stable",
        ...
        logger.exception(f"Feedback Agent error in get_faculty_analytics: {e}")
        return {"error": f"Failed to get faculty analytics: {str(e)}"}


def get_course_analytics(db: Session, course_id: int) -> Dict:
    """Course-level feedback analytics."""
    logger.info(f"Feedback Agent: get_course_analytics for course_id={course_id}")
    try:
        ...
        logger.exception(f"Feedback Agent error in get_course_analytics: {e}")
...
def get_platform_summary(db: Session) -> Dict:
    ...
    logger.info("Feedback Agent: get_platform_summary")
    try:
        all_responses = db.query(FeedbackResponse).all()
        total = len(all_responses)
        if total == 0:
            logger.info("Feedback Agent: No platform feedback responses found.")
            return {
                "total_responses": 0,
                "avg_platform_rating": 0.0,
                "sentiment_distribution": {"Positive": 0, "Negative": 0, "Neutral": 0},
                "sentiment_pct": {"Positive": 0.0, "Negative": 0.0, "Neutral": 0.0},
                "top_rated_faculty": [],
        ...
        top_faculty = []
        sorted_faculty = sorted(faculty_ratings.items(), key=lambda x: sum(x[1])/len(x[1]), reverse=True)[:5]
        
        for fid, ratings in sorted_faculty:
        ...
        logger.exception(f"Feedback Agent error in get_platform_summary: {e}")
        return {
            "total_responses": 0,
            "avg_platform_rating": 0.0,
            "sentiment_distribution": {"Positive": 0, "Negative": 0, "Neutral": 0},
